...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.prompt1 = ActiveBlock_Prompt_Basis(dim=dim*2**1, num_task=2, num_basis=64, embed_dim=64)
            self.prompt2 = ActiveBlock_Prompt_Basis(dim=dim*2**2, num_task=2, num_basis=32, embed_dim=128)
            self.prompt3 = ActiveBlock_Prompt_Basis(dim=dim*2**3, num_task=2, num_basis=16, embed_dim=320)
